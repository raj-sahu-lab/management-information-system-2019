...
class ADODB_ado extends ADOConnection {
	...
	//
	// $DB->Connect('User ID=[DB_USER];Password=[DB_PASSWORD];SERVER=mangrove;DATABASE=ai',false,false,'SQLOLEDB');
	...
	//	- 	if $argProvider is not empty, then $argDBorProvider holds db
		...
		if ($argProvider=='mssql') {
			...
			// not yet
			//if ($argDatabasename) $argHostname .= ";Initial Catalog=[DB_NAME]";
