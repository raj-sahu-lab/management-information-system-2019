...
class ADODB_postgres64 extends ADOConnection{
	...
	*/
	...
	//
	// examples:
	// 	$db->Connect("host=host1 user=user1 Password=[DB_PASSWORD]");
	...
	function _connect($str,$user='',$pwd='',$db='',$ctype=0)
	{
		...
		if ($user || $pwd || $db) {
			...
			if ($pwd)  $str .= " Password=[DB_PASSWORD]".$pwd;
			if ($db)   $str .= " dbname=".$db;
		}

		//if ($user) $linea = "user=$user host=$linea Password=[DB_PASSWORD]";
	...
	//
	// examples:
	// 	$db->PConnect("host=host1 user=user1 Password=[DB_PASSWORD]");
