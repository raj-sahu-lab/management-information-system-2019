  ...
  $log->debug("Exiting get_contactsforol method ...");
	...
	*/
	...
	function get_payments($id, $cur_tab_id, $rel_tab_id, $actions=false){
 		...
 		global $log, $singlepane_view,$currentModule,$current_user;
		$log->debug("Entering get_UserSubscriptions(".$id.") method ...");
		$this_module = $currentModule;
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
 		...
 		$query = "SELECT vtiger_crmentity.*, vtiger_payment.*, ". 
 		" case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name " .
		" FROM vtiger_payment ".
		" INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_payment.paymentid ".
		" INNER JOIN vtiger_contactdetails ON vtiger_contactdetails.contactid = vtiger_payment.parentid ".
		" LEFT JOIN vtiger_groups	ON vtiger_groups.groupid = vtiger_crmentity.smownerid " .
		" LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid = vtiger_users.id ".
		" WHERE vtiger_crmentity.deleted = 0 AND vtiger_contactdetails.contactid = ".$id." GROUP BY vtiger_payment.paymentid ";
		...
		$log->debug("Exiting get_UserSubscriptions method ...");
		return $return_value;
 	}
