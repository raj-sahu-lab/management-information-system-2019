...
{if $MODULE eq 'SalesOrder'}	{assign var="IS_INDIVIDUAL_TAX_TYPE" value=true}	{assign var="IS_GROUP_TAX_TYPE" value=false}{else}	{assign var="IS_INDIVIDUAL_TAX_TYPE" value=false}	{assign var="IS_GROUP_TAX_TYPE" value=true}{/if}{if $TAX_TYPE eq 'group'}	{assign var="IS_GROUP_TAX_TYPE" value=true}	{assign var="IS_INDIVIDUAL_TAX_TYPE" value=false}{/if}
		...
		<div>
			<div>
				{if $PRODUCT_ACTIVE eq 'true' && $SERVICE_ACTIVE eq 'true'}
					<div class="btn-toolbar">						{if $MODULE eq 'SalesOrder'}							<span class="btn-group">								<button type="button" class="btn btn-default" id="addService" data-module-name="Services" >									<i class="fa fa-plus"></i>&nbsp;&nbsp;<strong>{vtranslate('LBL_ADD_SERVICE',$MODULE)}</strong>								</button>							</span>						{else}
